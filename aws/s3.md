...
## AWS S3 Encryption & Snow Family: Top 1% Insights
...
- **SSE-S3 (AES-256 managed by AWS):**
  - Easy to enable, fully managed.
  - Top engineers know it doesn’t protect data in transit — always use HTTPS.
- **SSE-KMS (AWS KMS managed keys):**
  - Provides fine-grained access control and audit logs.
  - Costs can increase with frequent key usage — engineers batch operations to reduce KMS requests.
- **SSE-C (Customer-provided keys):**
  - Gives full control over keys.
  - Must manage key rotation and storage securely; losing a key means permanent data loss.
- **Top secrets:**
  - Combining SSE with **Bucket Policies** for mandatory encryption prevents accidental unencrypted uploads.
  - Using **encryption context in KMS** ensures only intended applications can decrypt objects.

#### Client-Side Encryption (CSE)

- Encrypt before uploading to S3.
- Engineers often combine CSE with SSE for **double-layer security**, especially for regulatory compliance.
- **Key management is critical:** Using AWS SDK envelope encryption properly avoids accidental exposure.

---

### AWS Snow Family

- **Snowball**: Petabyte-scale data transfer into/out of AWS.
  - Top engineers know how to use multiple devices in parallel for massive migrations.
- **Snowball Edge**: Compute + storage at the edge.
  - Supports Lambda functions; can run offline processing before uploading to S3.
- **Snowcone**: Smallest form factor for remote/edge sites.
  - Ideal for disconnected environments or harsh locations.
- **Secrets only top engineers use:**
  - Combining Snow devices with S3 multipart uploads dramatically improves throughput.
  - Pre-signed URLs can be used to automate device uploads securely.
  - Edge computing with Snowball Edge reduces data transfer costs and latency before pushing to central S3.
